...
from segment_anything_training.modeling.feature_vis import visualize_encoder_features,visualize_feature_map,visualize_feature_energy,visualize_instance_tokens,visualize_token_similarity_matrix,visualize_encoder_features_withInstanceID,compute_feature_structure_metrics,compute_significant_eigenvalues_with_mask
...
DEFAULT_SAM_CHECKPOINT = PROJECT_ROOT / "sam-hq-param" / "sam_vit_b_01ec64"
...
def unpack_main_decoder_outputs(outputs, expect_final_embedding):
    if expect_final_embedding:
        ...
        if len(outputs) == 3:
            ...
            masks_hq, masks_edge, masks_instance, instance_token_afer_fusion = outputs
        else:
            raise ValueError("Unexpected decoder output structure")
        upscaled_embedding_ours = None
    return masks_hq, masks_edge, masks_instance, upscaled_embedding_ours, instance_token_afer_fusion
...
def get_args_parser():
    ...
    parser.add_argument("--restore_model", type=str, required=True,
                        help="decoder checkpoint for evaluation")
    parser.add_argument("--checkpoint_vit", type=str, required=True,
                        help="multispectral encoder / LoRA checkpoint")
    ...
    parser.add_argument("--continue_sample_list", type=str, default=None)
...
#     sam = sam_model_registry_channelToken[args.model_type](checkpoint=args.checkpoint,checkpoint_vit=args.checkpoint_vit,use_lora=args.use_lora,use_orth_loss=args.use_orth_loss,in_channels=args.in_channels,args=args)
...
def main(net, test_datasets, args):
    ...
    if not args.eval:
        print("--- create testing dataloader ---")
        test_dataloaders, test_datasets = create_dataloaders_crop(
            test_datasets,
            batch_size=args.batch_size_test,
            training=False,
            data_type=args.data_type,
    ...
    # 模型加载到指定GPU
    net = net.to(device)
    sam = sam_model_registry_channelToken[args.model_type](
        checkpoint=args.checkpoint,
        checkpoint_vit=args.checkpoint_vit,
    ...
    # 加载权重
    print("restore model from:", args.restore_model)
    state_dict = torch.load(args.restore_model, map_location=device)
...
def evaluate(args, net, sam, valid_dataloaders, visualize=False):
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device("cpu")
    # sam_rgb = sam_model_registry['vit_b'](checkpoint=args.checkpoint).to('cuda')
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for step,data_val in progress_bar:
            ...
            imgs = inputs_val.permute(0, 2, 3, 1)
            batched_input = []
            ...
            (
                ...
                instance_token_afer_fusion,
            ) = unpack_main_decoder_outputs(
                masks_hq, args.struct_loss_in_finalEmbedding
            )
            # if args.cal_feature_structure:
            #         late_res = compute_feature_structure_metrics(encoder_embedding_h,masks_hq,data_val['image_name'][-1])
            #         early_res = compute_feature_structure_metrics(interm_embeddings_h[0].permute(0,3,1,2),masks_hq,data_val['image_name'][-1])
            #         early_metrics_rgb.append(early_res)
            #         late_metrics_rgb.append(late_res)
            if args.cal_feature_structure:
            ...
            if args.tnse_visualize:
                ...
                # out_path_rgb = '/mnt/disk3/har/Param/Cropland/A_ViT_FeatureVis/AI4Boundary/ChannelToken_AlignSAM_clp/SAM_RGB'
                # visualize_feature_map(encoder_embedding_h,out_path_rgb,data_val['image_name'][-1])

                # out_path = '/mnt/disk3/har/Param/Cropland/Ablations_channelToken/Pre_Result/channelToken_ep5/CroplandVis'
                out_path = os.path.join(args.output, 'CroplandVis')
                os.makedirs(out_path, exist_ok=True)

                # visualize_feature_map(encoder_embedding,out_path,data_val['image_name'][-1])
                visualize_feature_map(upscaled_embedding_ours,out_path,data_val['image_name'][-1])
                out_path = os.path.join(args.output, 'energy')
                os.makedirs(out_path, exist_ok=True)
                visualize_feature_energy(upscaled_embedding_ours,out_path,data_val['image_name'][-1])
                # tnse 
                ...
                masks_gt_bin = data_val['mask_semantic'].squeeze(1)
                visualize_encoder_features(encoder_embedding,masks_gt_bin,out_path,data_val['image_name'])
                # visualize_encoder_features_withInstanceID(encoder_embedding,masks_instance_gt,out_path,data_val['image_name'])
                # visualize_encoder_features_withInstanceID(encoder_embedding_h,masks_instance_gt,out_path_rgb,data_val['image_name'])
                # for i in range(len(interm_embeddings)):
                #     interm_name = ['interm_'+str(i)+'_'+item for item in data_val['image_name']]
                #     visualize_encoder_features(interm_embeddings[i].permute(0,3,1,2),masks_bin,out_path,interm_name)
                #     visualize_encoder_features_withInstanceID(interm_embeddings[i].permute(0,3,1,2),masks_instance_gt,out_path,interm_name)
                    # visualize_encoder_features_withInstanceID(interm_embeddings_h[i].permute(0,3,1,2),masks_instance_gt,out_path_rgb,interm_name)
            ...
            #     break
        if args.cal_feature_structure:
            # save_excel_imbedding_metrics(early_metrics_rgb, late_metrics_rgb,os.path.join(args.output,'rgb_feature_structure_metrics.xlsx'))
            save_excel_imbedding_metrics(early_metrics, late_metrics,os.path.join(args.output,'feature_structure_metrics.xlsx'))
        print('============================')
...
if __name__ == "__main__":
    ...
    dataset_s4a = {"name": "AI4Boundaries",
