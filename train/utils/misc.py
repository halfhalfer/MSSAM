...
def masks_to_boxes(masks,threshold=128):
    ...
    # mask_max = masks.max()
    # if mask_max == 0:
    x_mask = ((masks>threshold) * x.unsqueeze(0))
    x_max = x_mask.flatten(1).max(-1)[0]
    x_min = x_mask.masked_fill(~(masks>threshold), 1e8).flatten(1).min(-1)[0]

    y_mask = ((masks>threshold) * y.unsqueeze(0))
    y_max = y_mask.flatten(1).max(-1)[0]
    y_min = y_mask.masked_fill(~(masks>threshold), 1e8).flatten(1).min(-1)[0]
...
def masks_sample_points(masks,k=10,threshold=128):
    """
    ...
    for b_i in range(len(masks)):
        select_mask = (masks[b_i]>threshold)
...
def masks_noise(masks,mask_max=255):
    ...
    if mask_max == 255:
        ...
        gt_masks_vector = masks / mask_max
    ...
    if mask_max == 255:
        ...
        gt_masks_vector = gt_masks_vector * mask_max

    return gt_masks_vector


def mask_iou(pred_label,label,threshold=128):
    '''
    calculate mask iou for pred_label and gt_label
    '''

    pred_label = (pred_label>0)[0].int()
    label = (label>threshold)[0].int()
...
def boundary_iou(gt, dt, dilation_ratio=0.02,threshold=128):
    """
    ...
    gt = (gt>threshold)[0].cpu().byte().numpy()
